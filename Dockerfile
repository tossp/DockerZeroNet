...
RUN apk update && apk upgrade && \    
    ...
    rm f.zip &&\
    npm cache clean &&\
    mkdir -p /app/root/zeronet &&\
    ln -sv /root/zeronet/data/ /app/root/zeronet &&\
...
ENTRYPOINT ["python", "/app/zeronet.py","--config_file","/etc/zeronet.conf","--data_dir","/root/zeronet/data", "--log_dir","/root/zeronet/log", "--ui_ip", "0.0.0.0"]
CMD ["--debug"]
